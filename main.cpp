...
class Tracker_optflow {
  public:
    ...
    std::vector<coord_t> tracking_flow(cv::Mat new_dst_mat, bool check_error = true)
    {
        ...
        ////sync_PyrLKOpticalFlow_gpu.sparse(src_grey_gpu, dst_grey_gpu, prev_pts_flow_gpu, cur_pts_flow_gpu,
        /// status_gpu, &err_gpu);    // OpenCV 2.4.x
...
int main(int argc, char *argv[])
{
    ...
    try {
            ...
            for (cv::Mat frame; cap >> frame, cap.isOpened() && !frame.empty();) {
                ...
                if (frame_counter++ >= save_each_frames) { // save frame for each 3 second
                    ...
                    if (backward)
                        ...
                        image_counter++;
                    std::string img_name =
                        images_path + "/" + filename_without_ext + "_" + image_counter_ss.str() + ".jpg";
        ...
        // glob(images_path, filenames_in_folder); // void glob(String pattern, std::vector<String>& result, bool
        // recursive = false);
        cv::String              images_path_cv = images_path;
        std::vector<cv::String> filenames_in_folder_cv;
        glob(images_path_cv,
            filenames_in_folder_cv); // void glob(String pattern, std::vector<String>& result, bool recursive = false);
        ...
        for (auto &i : filenames_in_folder) {
            ...
            if (ext == "jpg" || ext == "JPG" || ext == "jpeg" || ext == "JPEG" || ext == "bmp" || ext == "BMP" ||
                ext == "png" || ext == "PNG" || ext == "ppm" || ext == "PPM") {
        ...
        auto dif_it_end = std::set_difference(jpg_filenames_without_ext.begin(), jpg_filenames_without_ext.end(),
            txt_filenames.begin(), txt_filenames.end(), difference_filenames.begin());
        ...
        auto inter_it_end = std::set_intersection(jpg_filenames_without_ext.begin(), jpg_filenames_without_ext.end(),
            txt_filenames.begin(), txt_filenames.end(), intersect_filenames.begin());
        ...
        for (auto &i : intersect_filenames) {
            size_t ext_index = find(jpg_filenames_without_ext.begin(), jpg_filenames_without_ext.end(), i) -
        ...
        do {
            ...
            if (old_trackbar_value != trackbar_value || exit_flag) {
                ...
                if (old_trackbar_value >=
                    0) // && current_coord_vec.size() > 0) // Yolo v2 can processes background-image without objects
                {
                    try {
                        ...
                        for (auto &i : current_coord_vec) {
                            float const relative_center_x =
                                (float)(i.abs_rect.x + i.abs_rect.width / 2) / full_image_roi.cols;
                            float const relative_center_y =
                                (float)(i.abs_rect.y + i.abs_rect.height / 2) / full_image_roi.rows;
                            ...
                            ofs << i.id << " " << relative_center_x << " " << relative_center_y << " " << relative_width
                                << " " << relative_height << std::endl;
                        }

                        // store [path/image name.jpg] to train.txt
                        auto it =
                            std::find(difference_filenames.begin(), difference_filenames.end(), filename_without_ext);
                        if (it != difference_filenames.end()) {
                            ofs_train << images_path << "/" << jpg_filename << std::endl;
                            ofs_train.flush();

                            size_t new_size = std::remove(difference_filenames.begin(), difference_filenames.end(),
                ...
                for (size_t i = 0; i < preview_number && (i + trackbar_value) < jpg_filenames_path.size(); ++i) {
                    ...
                    putText(dst_roi, jpg_filenames[trackbar_value + i], Point2i(0, 10), FONT_HERSHEY_COMPLEX_SMALL, 0.5,
                        Scalar::all(255));
                    ...
                    if (i == 0) {
                        ...
                        try {
                            ...
                            std::string const txt_filename =
                                jpg_filename.substr(0, jpg_filename.find_last_of(".")) + ".txt";
                            ...
                            for (std::string line; getline(ifs, line);) {
                                ...
                                float relative_coord[4] = {
                                    -1, -1, -1, -1}; // rel_center_x, rel_center_y, rel_width, rel_height
                                ...
                                for (size_t i = 0; i < 4; i++)
                                    if (relative_coord[i] < 0)
                                        continue;
                                coord.abs_rect.x =
                                    (relative_coord[0] - relative_coord[2] / 2) * (float)full_image_roi.cols;
                                coord.abs_rect.y =
                                    (relative_coord[1] - relative_coord[3] / 2) * (float)full_image_roi.rows;
                    ...
                    if (!std::binary_search(
                            difference_filenames.begin(), difference_filenames.end(), filename_without_ext)) {
            ...
            if (selected) {
                ...
                if (y_end < preview.rows && x_end > prev_img_rect.width &&
                    x_end < (full_image.cols - prev_img_rect.width) && y_start < preview.rows) {
                    ...
                    Rect selected_rect(
                        Point2i((int)min(x_start, x_end), (int)min(y_start, y_end)), Size(x_size, y_size));
            ...
            if (show_mouse_coords) {
                ...
                char        buff[100];
                snprintf(
                    buff, 100, "Abs: %d x %d    Rel: %.3f x %.3f", abs_x, abs_y, relative_center_x, relative_center_y);
            ...
            // Draw crosshair
            {
                ...
                if (y_end >= offset) {
                    ...
                    const int  ver_min =
                        draw_select ? std::min(x_end - mouse_offset, x_start - mouse_offset) : x_end - mouse_offset;
                    const int ver_max =
                        draw_select ? std::max(x_end + mouse_offset, x_start + mouse_offset) : x_end + mouse_offset;
                    const int hor_min =
                        draw_select ? std::min(y_end - mouse_offset, y_start - mouse_offset) : y_end - mouse_offset;
                    const int hor_max =
                        draw_select ? std::max(y_end + mouse_offset, y_start + mouse_offset) : y_end + mouse_offset;
                    ...
                    cv::line(crosshair_mask, cv::Point(0, y_end), cv::Point(ver_min, y_end),
                        cv::Scalar(bit_high)); // Horizontal, left to mouse
                    cv::line(crosshair_mask, cv::Point(ver_max, y_end), cv::Point(crosshair_mask.size().width, y_end),
                        cv::Scalar(bit_high)); // Horizontal, mouse to right
                    cv::line(crosshair_mask, cv::Point(x_end, offset), cv::Point(x_end, std::max(offset, hor_min)),
                        cv::Scalar(bit_high)); // Vertical, top to mouse
                    cv::line(crosshair_mask, cv::Point(x_end, hor_max), cv::Point(x_end, crosshair_mask.size().height),
            ...
            for (size_t k = 0; k < current_coord_vec.size(); ++k) {
                ...
                if (show_mark_class) {
                    putText(full_image_roi, std::to_string(i.id) + synset_name, i.abs_rect.tl() + Point2f(2, 22),
                        FONT_HERSHEY_SIMPLEX, 0.8, color_rect, 2);
                }
            ...
            if (next_by_click) {
                putText(full_image_roi, "Mode: 1 mark per image (next by click)", Point2i(850, 20),
                    FONT_HERSHEY_SIMPLEX, 0.6, Scalar(50, 170, 100), 2);
            }
            ...
            if (show_help) {
                putText(full_image_roi,
                    "<- prev_img    -> next_img    c - clear_marks    n - one_object_per_img    0-9 - obj_id    m - "
                    "show coords    ESC - exit",
                    Point2i(0, 45), FONT_HERSHEY_SIMPLEX, 0.6, Scalar(50, 10, 10), 2);
                putText(full_image_roi,
                    "w - line width   k - hide obj_name   p - copy previous   o - track objects   r - delete selected  "
                    " R-mouse - move box", //   h - disable help",
                    Point2i(0, 80), FONT_HERSHEY_SIMPLEX, 0.6, Scalar(50, 10, 10), 2);
            } else {
                putText(
                    full_image_roi, "h - show help", Point2i(0, 45), FONT_HERSHEY_SIMPLEX, 0.6, Scalar(50, 10, 10), 2);
            }
            ...
            if (pressed_key >= '0' && pressed_key <= '9')
                current_obj_id = current_obj_id * 10 + pressed_key - '0'; // 0 - 9
            if (pressed_key >= 1048624 && pressed_key <= 1048633)
                current_obj_id = current_obj_id * 10 + pressed_key - 1048624; // 0 - 9
            if (pressed_key >= 1114032 && pressed_key <= 1114041)
                current_obj_id = current_obj_id * 10 + pressed_key - 1114032; // 0 - 9
